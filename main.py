    ...
    for team in teams:
        for player in team:
            if(isinstance(player, list)):
                #print(player[2]) 
                player[2] = two_opt(player[2], instace, sizePayer)
                #print(player[2])
    ...
    while improved:
        ...
        for i in range(1, sizePayer-2):
            for j in range(i+1, sizePayer):
                ...
                if score_relax(instace, new_player) > score_relax(instace, best):
                    best = new_player
                    improved = True
        player = best
    return best  
...
if __name__ == '__main__':
    ...
    for k in range(5):
        ...
        '''for i in teams:
            for j in i:
                if(k==99 and isinstance(j, list)):
                    score_relax(instace, j)
